...
public class WhitelistRemoveMenu {
    ...
    public static void openWhitelistRemoveMenu(Player player, Region region) {
        String title = config.getString("regions.gui.whitelist-remove.title").replace("{region}", region.getName());
        int rows = config.getInt("regions.gui.whitelist-remove.rows");
    ...
    private static void addControlButtons(PaginatedGui gui, Player player, Region region) {
        ...
        HashSet<Integer> backSlots = ItemStackUtils.getSlots(config.getString("regions.gui.whitelist-remove.back.slot"));
    ...
    private static void addPaginationControls(PaginatedGui gui) {
        ...
        HashSet<Integer> previousPageSlots = ItemStackUtils.getSlots(config.getString("regions.gui.whitelist-remove.previous-page.slot"));
        ...
        HashSet<Integer> nextPageSlots = ItemStackUtils.getSlots(config.getString("regions.gui.whitelist-remove.next-page.slot"));
    ...
    private static void addPlayerItems(PaginatedGui gui, Player player, Region region) {
        List<String> players = getPlayers(region);
        for (String target : players) {
            ItemStack playerItem = getPlayerItem(target);
            gui.addItem(new GuiItem(playerItem, event -> {
                try {
                    region.removeWhitelistedPlayer(target);
                    MessageUtils.sendMessage(player, "whitelist.removed", new Pair<>("{region}", region.getName()), new Pair<>("{player}", target));
    ...
    private static ItemStack getPlayerItem(String player) {
        String materialName = config.getString("regions.gui.whitelist-remove.player.item");
        Material material = Material.getMaterial(materialName.toUpperCase());

        String title = config.getString("regions.gui.whitelist-remove.player.title").replace("{player}", player);
        List<String> lore = config.getStringList("regions.gui.whitelist-remove.player.lore");
    ...
    private static ItemStack getControlItem(String key) {
        String materialName = config.getString("regions.gui.whitelist-remove." + key + ".material");
        Material material = Material.getMaterial(materialName.toUpperCase());

        String title = config.getString("regions.gui.whitelist-remove." + key + ".title");
        List<String> lore = config.getStringList("regions.gui.whitelist-remove." + key + ".lore");
    ...
    private static List<String> getPlayers(Region region) {
        return region.getWhitelistedPlayers();
    }
}